...
export function CompanyForm() {
  ...
  const handleSubmit = (values: CompanyFormValues) => {
    try {
      const message = `*PROPOSTA DE PARCERIA - AGENCIA DE TURISMO*
      ...
      <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-4 sm:space-y-6">
          ...
          <Card className="border-amber-200 bg-amber-50/50">
            <CardContent className="pt-4 sm:pt-6">
              <p className="text-amber-800 text-sm sm:text-base">
                Oferecemos condições especiais para agências que desejam incluir nosso espaço em
                seus pacotes turísticos. Temos estrutura completa para grupos, day use, eventos e
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              {' '}
              <FormLabel>Proposta de Parceria (Opcional)</FormLabel>
              <FormControl>
                <Textarea
              ...
              <FormDescription>
                Conte-nos sobre seu interesse em oferecer nosso espaço aos seus clientes - day use,
                eventos, hospedagem, etc.
              </FormDescription>
              <FormMessage />
            </FormItem>
          )}
