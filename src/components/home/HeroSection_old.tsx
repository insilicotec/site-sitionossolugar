...
import { HERO_IMAGES, LOGO_ASSETS } from '@/assets/constants/images';
import { FEATURE_HIGHLIGHTS } from '@/assets/constants/navigation';
...
const HeroSection = () => {
  return (
    <section className="relative pt-20 md:pt-28 min-h-[85vh] md:min-h-[90vh] flex items-center justify-center overflow-hidden bg-gradient-to-br from-white via-amber-50/30 to-amber-100/50">
      {/* Background subtle pattern */}
      <div className="absolute inset-0 opacity-5">
        <div
          className={`absolute inset-0 bg-[url('${HERO_IMAGES.background}')] bg-cover bg-center`}
        ></div>
      </div>
      ...
      <div className="container relative z-10 px-4 md:px-6 py-12 md:py-20 mx-auto">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <p className="max-w-3xl mx-auto mb-8 md:mb-10 text-base md:text-lg lg:text-xl text-gray-700 leading-relaxed px-4 md:px-0">
            ...
            <span className="text-amber-800 font-semibold">seus momentos mais especiais</span>, onde
            a natureza e o conforto se encontram para criar experiências inesquecíveis.
          </p>
          {/* Feature highlights */}
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-4 max-w-4xl mx-auto mb-8 md:mb-10 px-2 md:px-0">
            {FEATURE_HIGHLIGHTS.map(feature => (
              <div
                key={feature.id}
                className="bg-white/80 backdrop-blur-sm p-4 md:p-5 rounded-xl border border-amber-200/50 flex items-center space-x-3 md:space-x-4"
              >
